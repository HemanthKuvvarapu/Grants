...
public class GlobalFuncAmendment
{
    ...
    public static bool isValidNumOfStaffFTEAmendment(List<string> errorBoxes, DataSet ds)
    {
        ...
        if (Single.Parse(dr["FringeBenefitsNumofStaff"].ToString().Trim()) + Single.Parse(string.IsNullOrEmpty(drNew["FringeBenefitsNumofStaff"].ToString().Trim()) ? "0.00" : drNew["FringeBenefitsNumofStaff"].ToString().Trim()) <
            Single.Parse(dr["FringeBenefitsFTE"].ToString().Trim()) + Single.Parse(string.IsNullOrEmpty(drNew["FringeBenefitsFTE"].ToString().Trim()) ? "0.00" : drNew["FringeBenefitsFTE"].ToString().Trim()))
            errorBoxes.Add("txtFringeBenefitsFTE");
    ...
    public static bool isValidBudgetAmendmentUpToEligibilityAmount(List<string> errorBoxes, DataSet ds)
    {
        int count = errorBoxes.Count;

        DataRow dr = ds.Tables[3].Rows[0];

        decimal GrandTotal = Decimal.Parse(dr["BudgetTotalInt"].ToString());
        decimal AllocFund = 0;

        switch (dr["Version"].ToString())
        {

            case "1":
                AllocFund = Decimal.Parse(dr["OriginalBudgetAlloc"].ToString()) + Decimal.Parse(dr["Amendment1Eligibility"].ToString());
                break;
            case "2":
                AllocFund = Decimal.Parse(dr["OriginalBudgetAlloc"].ToString()) + Decimal.Parse(dr["Amendment1Eligibility"].ToString()) + Decimal.Parse(dr["Amendment2Eligibility"].ToString());
                break;
            case "3":
                AllocFund = Decimal.Parse(dr["OriginalBudgetAlloc"].ToString()) + Decimal.Parse(dr["Amendment1Eligibility"].ToString()) + Decimal.Parse(dr["Amendment2Eligibility"].ToString()) + Decimal.Parse(dr["Amendment3Eligibility"].ToString());
                break;
            case "":
                break;
        }
