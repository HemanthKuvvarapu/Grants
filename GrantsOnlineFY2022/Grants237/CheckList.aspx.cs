...
public partial class CheckList : System.Web.UI.Page
{
    ...
    private void DisplayPCHPSection()
    {
        if (Session["PCHPAgency"] != null && Session["PCHPAgency"].ToString() == "1")
        {
            blkPCHP1.Visible = true;
            blkPCHP2.Visible = true;
        }
        else
        {
            blkPCHP1.Visible = false;
            blkPCHP2.Visible = false;
        }
    }
    ...
    private void DisplayAmendmentSection()
    {
        if (AppInfo.Amendment1 || AppInfo.Amendment2 || AppInfo.Amendment3)
        {
			...
			btnSelectLeadAgencyBudgetPart4a.Enabled = false;
    ...
    private void BindData()
    {
        ...
        if (Session["PCHPAgency"] != null && Session["PCHPAgency"].ToString() == "1")
        {
            if (isValidPCHPBudget(out errorCount, out warningCount, out isNoRecord))
            {
                if (!isNoRecord)
                {
                    imgPart4a.ImageUrl = "~/Images/Completed.jpg";
                }
                else
                {
                    if (enableSubmit)
                ...
                lblErrorPart4a.Visible = false;
            }
            else
            {
                lblErrorPart4a.Visible = true;
                lblErrorPart4a.Text = "(" + errorCount.ToString() + " error(s) found)";
            ...
            if (warningCount > 0)
            {
                lblWarningPart4a.Visible = true;
                lblWarningPart4a.Text = "(" + warningCount.ToString() + " warning(s) found)";
            }
            else
            {
                lblWarningPart4a.Visible = false;
            }
        }
        //end - part 4 validation
    ...
    private bool isValidBudgetSummary(out int errorCount, out bool isNoRecord)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            if (!GlobalFunc.isValidBudgetSummaryEligibilityAmount(Int32.Parse(Single.Parse(ds.Tables[0].Rows[0]["EligibilityAmount"].ToString()).ToString()), ds))
            {
                errorCount++;
            }
    ...
    private bool isValidPCHPBudget(out int errorCount, out int warningCount, out bool isNoRecord)
    {
        errorCount = 0;
        warningCount = 0;
        isNoRecord = false;

        if (Session["PCHPAgency"] == null || Session["PCHPAgency"].ToString() == "0")
        {
            return true;
        }
        else
        {
            ...
            DataSet ds = dal.GetBudgetPCHPNarrativeDataNew(Int32.Parse(Session["GrantApplicantId"].ToString()));
    ...
    protected void btnPCHPReport_Click(object sender, EventArgs e)
    {
        Response.Redirect("BudgetPCHPNarrative.aspx");
    }

    protected void btnSelectLeadAgencyBudgetPart4a_Click(object sender, EventArgs e)
    {
        Response.Redirect("BudgetPCHPNarrative.aspx");
    }
}
