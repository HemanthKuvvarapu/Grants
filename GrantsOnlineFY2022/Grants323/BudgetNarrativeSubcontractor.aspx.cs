...
public partial class BudgetNarrativeSubcontractor : System.Web.UI.Page
{
    ...
    private void DisableColumn()
    {
        ...
        {
            if (!DisabledColumn.ContainsKey(ds.Tables[0].Rows[i]["ColumnName"].ToString()))
            {
    ...
    private void BindData(int BudgetNarrativeID, bool isAfterSaved)
    {
        if (BudgetNarrativeID != -1)
        {
            ds = dal.GetHeadStartBudgetNarrativeSubcontractorData(BudgetNarrativeID, 1);
