...
public partial class CheckList : System.Web.UI.Page
{
    ...
    private void DisplayAmendmentSection()
    {
        if (AppInfo.Amendment1 || AppInfo.Amendment2 || AppInfo.Amendment3)
        {
            ...
            if (!AppInfo.EnableBothAmendmentAndOnline)
            {
    ...
    private void BindData()
    {
        ...
        {
            if (isValidSubcontractorBudget(out errorCount, out warningCount, out isNoRecord) && isNoRecord == true)
            {
        ...
        {
            if (isValidPDFCheckList(out errorCount, out isNoRecord) && errorCount == 0)
            {
                if (!isNoRecord)
                {
            ...
            {
                imgPart10.ImageUrl = "~/Images/Completed.jpg";
                //lblErrorPart10.Visible = true;

                //lblErrorPart10.Text = "(" + errorCount.ToString() + " error(s) found)";
    ...
    private bool isValidPDFCheckList(out int errorCount, out bool isNoRecord)
    {
        ...
        DataSet ds2 = dal.GetGrantPartStatus(Int32.Parse(Session["GrantApplicantId"].ToString()), "10");
