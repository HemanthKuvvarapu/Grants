...
public partial class CommunitiesServed : System.Web.UI.Page
{
    ...
    public string shouldDisplayTowns = "block";
    public string IsReadOnly = "ReadOnly";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (AppInfo.FundCode == 325 && AppInfo.FiscalYear == 2019)
        {
            shouldDisplayTowns = "none";
        }
        //BindScripts();
    }
    ...
    private void DisplayForMentalHealth()
    {
        ...
        trCensusChildren0to5.Visible = true;
        trCensusHouseholds.Visible = false;
        trAvgProvidersFamilies.Visible = true;
        trLicensedProviders.Visible = true;
    ...
    private void DisplayForCareerPathways()
    {
        trEnglishLanguageLearner.Visible = true;
        trFirstLanguageNotEnglish.Visible = true;
        trAvgProvidersFamilies.Visible = true;
    ...
    private float GetCheckedRegionFunding()
    {
        CheckBox chkSelectAll;
        float funding = 0f;

        for (int i=0; i < dlRegionList.Items.Count; i++)
        {
            chkSelectAll = (CheckBox) dlRegionList.Items[i].FindControl("chkSelectAll");

            if (chkSelectAll.Checked)
            {
                funding += GetRegionFunding(i);
            }
        }
    ...
    private void SaveData()
    {
        try
        {
            ...
            if (AppInfo.EligibilityAmountFromTowns)
            {
                dal.UpdateEligibilityAmount(Convert.ToInt32(Session["GrantApplicantId"]), 0, AppInfo.FiscalYear, AppInfo.FundCode, 1);
            ...
            if (AppInfo.FundCode == 325 && AppInfo.FiscalYear == 2019)
            {
                float eligAmount = GetCheckedRegionFunding();

                dal.UpdateEligibilityAmount(Convert.ToInt32(Session["GrantApplicantId"]), eligAmount, AppInfo.FiscalYear, AppInfo.FundCode, 0);
    ...
    private float GetRegionFunding(int index)
    {
        if (index == 0)
        {
            return 995987.80f;
