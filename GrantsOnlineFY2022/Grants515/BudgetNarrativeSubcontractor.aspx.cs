...
public partial class BudgetNarrativeSubcontractor : System.Web.UI.Page
{
    DAL dal = new DAL();
    DataSet ds;
    DataSet ds2;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            //Need to pre-populat4 th
            LoadSubcontractorList();

            string BudgetNarrativeID = (Request.QueryString["id"] == null) ? "" : Request.QueryString["id"].ToString();
            string SystemProviderID = (Request.QueryString["sid"] == null) ? "-1" : Request.QueryString["sid"].ToString();
            ...
            if (!string.IsNullOrEmpty(SystemProviderID))
            {
                ViewState["SystemProviderId"] = SystemProviderID;
            }

            BindData((string.IsNullOrEmpty(BudgetNarrativeID)?-1:Int32.Parse(BudgetNarrativeID)), false);
    ...
    {
        if (ddlSubcontractorName.Items.Count == 1)
        {
            DataSet dsSubcontractorList = dal.GetCPPISystemProvidersNew(Convert.ToInt32(Session["GrantApplicantId"].ToString()), AppInfo.FiscalYear);
            if (dsSubcontractorList.Tables.Count > 0)
            {
                //ddlSubContractorName.Items.Insert(0, new ListItem(" ", "-1"));
                //if (ddlSubcontractorName.Items.Count > 0) AgencyList.Items.Clear();
                ddlSubcontractorName.DataSource = dsSubcontractorList.Tables[0];
                ddlSubcontractorName.DataTextField = dsSubcontractorList.Tables[0].Columns["txtProgramName"].ColumnName.ToString();
                ddlSubcontractorName.DataValueField = dsSubcontractorList.Tables[0].Columns["SystemProviderId"].ColumnName.ToString();
                ddlSubcontractorName.DataBind();

                ddlSubcontractorName.Items[0].Selected = true;
            }
            //else
            //{
            //    UserMsg.Text = sError;
        ...
        {
            ddlSubcontractorName.SelectedIndex = 0;
        }
    ...
    private int SaveBudgetNarrativeSubcontractorData()
    {
        ...
        if (Page.IsValid)
        {
            ...
            if (ViewState["BudgetNarrativeID"] != null)
            {
                BudgetNarrativeID = Int32.Parse(ViewState["BudgetNarrativeID"].ToString());
            }
            ...
            BudgetNarrativeID = dal.SaveBudgetNarrativeData((ViewState["BudgetNarrativeID"] == null) ? null : ViewState["BudgetNarrativeID"].ToString(),
                                                                         ...
                                                                         ddlSubcontractorName.SelectedItem.Text,
                                                                         txtSubcontractorEmail.Text,
                                                                         txtSubcontractorPhone.Text,
                                                                         txtSubcontractorContact.Text,
    ...
    protected void btnSaveHome_Click(object sender, EventArgs e)
    {
        int BudgetNarrativeID = -1;
        //string errorMsg = "";
        ContactID = Convert.ToInt32(hidContactID.Value);

        //Response.Write("1 ContactID=" + ContactID.ToString() + "<br/>");

        try
        {
            BudgetNarrativeID = SaveBudgetNarrativeSubcontractorData();

            if (BudgetNarrativeID > -1)
            {
                ViewState["BudgetNarrativeID"] = BudgetNarrativeID;
                ViewState["SystemProviderId"] = ddlSubcontractorName.SelectedValue.ToString();

                //Save Contact Data now
                ContactID = dal.SaveSubcontractorContactData(ContactID,
                                                Convert.ToInt32(Session["GrantApplicantId"].ToString()),
                                                //Convert.ToInt32(ddlSubcontractorName.SelectedValue),
                                                Convert.ToInt32(ViewState["SystemProviderId"].ToString()),
                                                BudgetNarrativeID,
                                                ddlSubcontractorName.SelectedItem.Text,
                                                txtSubcontractorContact.Text,
                                                txtSubcontractorAddress1.Text,
                                                txtSubcontractorAddress2.Text,
                                                txtSubcontractorCity.Text,
                                                txtSubcontractorState.Text,
                                                txtSubcontractorZipcode.Text,
                                                txtSubcontractorPhone.Text,
                                                txtSubcontractorFax.Text,
                                                txtSubcontractorEmail.Text,
                                                1); //IsFiscalContact

                tblSuccess.Visible = true;

                BindData(BudgetNarrativeID, true);
            }
        }
        catch (Exception ex)
        {
            ...
            //Response.Write("Error occurred while saving the data: " + ex.Message);
            //Response.End();
        }
    }
    
    protected void btnPrintHSSubBudget_Click(object sender, EventArgs e)
    {
        int BudgetNarrativeID = -1;
        //string errorMsg = "";
        ContactID = Convert.ToInt32(hidContactID.Value);
        ...
        try
        {
            ...
            {
                ViewState["BudgetNarrativeID"] = BudgetNarrativeID;
                Session["SCID"] = BudgetNarrativeID;
                //Save Contact Data now
                ContactID = dal.SaveSubcontractorContactData(ContactID,
                                                Convert.ToInt32(Session["GrantApplicantId"].ToString()),
                                                //Convert.ToInt32(ddlSubcontractorName.SelectedValue),
        ...
        Response.Redirect("Print.aspx?type=SubcontractorBudget");
    }
    ...
    private void BindData(int BudgetNarrativeID, bool isAfterSaved)
    {
        if (BudgetNarrativeID != -1)
        {
            //New lookup to get Subcontractor Budget and new Fiscal Contact data together
            ds = dal.GetContactAndBudgetNarrativeSubcontractorData(BudgetNarrativeID, Convert.ToInt32(Session["GrantApplicantID"].ToString()));
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                hidContactID.Value = dr["ContactID"].ToString();
                ContactID = Convert.ToInt32(dr["ContactID"].ToString());

                //txtSubcontractorName.Text = dr["AgencyName"].ToString();
                for (int i=0; i<ddlSubcontractorName.Items.Count; i++)
                {
                    if (ddlSubcontractorName.Items[i].Text.Trim().ToLower() == dr["AgencyName"].ToString().Trim().ToLower())
                    {
                        ddlSubcontractorName.SelectedIndex = i;
                    }
                }
                //ddlSubcontractorName.SelectedItem.Text = dr["AgencyName"].ToString();
                txtSubcontractorEmail.Text = dr["AgencyEmail"].ToString();
                txtSubcontractorPhone.Text = dr["AgencyPhone"].ToString();
                txtSubcontractorContact.Text = dr["AgencyContactName"].ToString();
                ...
                {
                    //Response.Write("<br/>3 ContactID=" + ContactID.ToString());
                    // Need to get additional fiscal Contact data for Subcontractors
                    ds2 = dal.GetSubcontractorContactData(Int32.Parse(ViewState["SystemProviderId"].ToString()), Int32.Parse(Session["GrantApplicantID"].ToString()), ContactID, 1); 

                    if (ds2.Tables[0].Rows.Count > 0)
                    {
                        DataRow dr2 = ds2.Tables[0].Rows[0];
                        ContactID = Int32.Parse(dr2["ContactID"].ToString());
                        //ddlSubcontractorName.SelectedItem.Text = dr2["SubcontractorName"].ToString();
                        txtSubcontractorEmail.Text = dr2["SubcontractorEmail"].ToString();
                        txtSubcontractorPhone.Text = dr2["SubcontractorPhone"].ToString();
                        txtSubcontractorContact.Text = dr2["SubcontractorContactName"].ToString();
                        txtSubcontractorFax.Text = dr2["SubcontractorFax"].ToString();
                        txtSubcontractorAddress1.Text = dr2["SubcontractorAddress1"].ToString();
                        txtSubcontractorAddress2.Text = dr2["SubcontractorAddress2"].ToString();
                        txtSubcontractorCity.Text = dr2["SubcontractorCity"].ToString();
                        txtSubcontractorState.Text = dr2["SubcontractorState"].ToString();
                        txtSubcontractorZipcode.Text = dr2["SubcontractorZipcode"].ToString();
                        //txtSubcontractorName.Text = dr2["SubcontractorName"].ToString();

                        ddlSubcontractorName.Enabled = false;
                        ddlSubcontractorName.Visible = true;
                        trProviderDDL.Visible = true;

                        //trProviderDDL.Visible = false;
                        //trProviderName.Visible = true;
                        //txtSubcontractorName.Visible = true;
    ...
    {
        Response.Redirect("BudgetNarrativeSummary.aspx");
    }
    ...
    protected void ddlSubcontractorName_SelectedIndexChanged(object sender, EventArgs e)
    {

    }

    protected void CustomValidator01_ServerValidate(object source, ServerValidateEventArgs args)
    {
        if (ddlSubcontractorName.SelectedValue == "0")
        {
            args.IsValid = false;
        }
    }
