...
public class GlobalFuncAmendment
{
    ...
    public static bool isValidBudgetUpToAmendmentSummaryEligibilityAmount(int AllocFund, DataSet ds)
    {
        DataRow dr = ds.Tables[0].Rows[0];
    ...
    public static bool isValidBudgetAmendmentUpToEligibilityAmount(List<string> errorBoxes, DataSet ds)//, string version)
    {
        int count = errorBoxes.Count;

        DataRow dr = ds.Tables[3].Rows[0];

        decimal GrandTotal = Decimal.Parse(dr["BudgetTotal"].ToString());
        decimal AllocFund = 0;
