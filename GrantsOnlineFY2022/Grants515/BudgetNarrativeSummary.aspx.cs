...
public partial class BudgetNarrativeSummary : System.Web.UI.Page
{
    ...
    private bool hasValidationErrors()
    {
        ...
        DataRow dr = dsSumm.Tables[0].Rows[0];

        if (!GlobalFunc.isValidBudgetUpToEligibilityAmount(Int32.Parse(Single.Parse(Session["Eligibility Amount"].ToString()).ToString()), dsSumm))
        {
            lstErrors.Items.Add("The total requested budget amount is greater than the maximum eligibility amount. Please revise your budget accordingly.");
            errorCount++;
        }

        // Send the Total Budget Amount, not the Max Eligibility Amount, to the next function
        if (!GlobalFunc.isValidBudgetSummaryEECAdminLimit(ds, Int32.Parse(dr["BudgetAmountTotal"].ToString()), out amount))
        {
            lstErrors.Items.Add("EEC's " + AppInfo.EECAdminLimitPerc.ToString() + "% Admin limit has been exceeded by " + "$" + amount.ToString("N0") + ". Please revise your budget accordingly.");
            errorCount++;
        }
        ...
        {
            if (Session["ProgramTypeId"].ToString() == "4") //only check if FCS
            {
                ...
                {
                    lstErrors.Items.Add("The Lead Agency's requested amount did not account for " + (AppInfo.UPKPassFundToProviderPerc * 100f).ToString("N0") + "% of their UPK funds being passed through to the FCC Providers.");
                    errorCount++;
                }
            }
        }
        ...
        if (AppInfo.FundCode == 391)
        {
            if (!GlobalFunc.IsValidTravel(Int32.Parse(Single.Parse(Session["Eligibility Amount"].ToString()).ToString()), ds))
            {
                lstErrors.Items.Add("Direct service cost allocated to Travel must not exceed 5% of the Total Grant Eligibility Amount, please revise the dollar amount.");
                errorCount++;
            }

            if (!GlobalFunc.IsValidSuppliesMaterials(Int32.Parse(Single.Parse(Session["Eligibility Amount"].ToString()).ToString()), ds))
            {
                lstErrors.Items.Add("Supplies and materials subtotal (for Educational Supplies and Materials and Instructional Technology including Software) must not exceed 5% of the Total Eligibility Amount, please revise the dollar amount.");
                errorCount++;
            }

            if (!GlobalFunc.IsValidCapacityBuilding(Int32.Parse(Single.Parse(Session["Eligibility Amount"].ToString()).ToString()), ds))
            {
                lstErrors.Items.Add("Capacity Building activities must not exceed 5% of the Total Eligibility Amount, please revise the dollar amount.");
                errorCount++;
            }
        }
        ...
        {
            lstErrors.Items.Add("EEC's 5% Ancillary Services has been exceeded by " + "$" + amount2.ToString("N0") + ". Please revise your budget accordingly.");
            errorCount++;
        }
    ...
    protected void grdViewAdmin_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            {
                // Get the SystemProviderId for the Agency and pass it to the next page
                int SystemProviderId = dal.GetCPPISystemProviderId(Convert.ToInt32(Session["GrantApplicantId"].ToString()), oRow["AgencyName"].ToString());

                oHyperLink.NavigateUrl = "BudgetNarrativeSubcontractor.aspx?id=" + oRow["BudgetNarrativeID"].ToString() + "&sid=" + SystemProviderId.ToString();
            }
        }
    }
